...
class_name JuliaViewer

@export var settings: VBoxContainer

@export var c = Vector2(0.506, -0.667)
...
func load_uniforms():
	...
	$TextureRect.material.set_shader_parameter("constant", c)
	...
	if mouse_inside:
		...
		if event is InputEventMouseMotion and drag:
			...
			if Input.is_action_pressed("set"):
				c.x = event.position.x / $TextureRect.size.x * 2.0 - 1.0
				c.y = -(event.position.y / $TextureRect.size.y * 2.0 - 1.0)
			else:
				pos += diff * zoom
...
func _process(delta):
	...
	settings.get_node("ConstantLabel").text = "c: %.3f + %.3fi" % [c.x, c.y]
